...
export async function GET() {
  try {
    ...
    if (existingId) {
      const { data, error } = await supabase.from("game_sessions").select("id, state").eq("id", existingId).eq("status", "active").maybeSingle();
      if (error) throw error;
      if (data) {
        const state = upgradeLegacyState(data.state as Partial<GameState>);
        if ((data.state as Partial<GameState>).version !== 2) await supabase.from("game_sessions").update({ state }).eq("id", data.id);
        return NextResponse.json({ state, sessionId: data.id });
      }
    }
    const { data, error } = await supabase.from("game_sessions").insert({ city: "mumbai", business_type: "cafe", state: INITIAL_STATE, status: "active" }).select("id, state").single();
