...
export async function POST(request: Request) {
  try {
    ...
    }) as GameState & { businessName?: string };
    ...
    await supabase.from("game_events").insert({ session_id: sessionId, day: 1, event_type: "business_setup", payload: { businessName, capital: state.capital, location: state.location, format: state.format, menu: state.menu, setupCost: state.setupCost } });
