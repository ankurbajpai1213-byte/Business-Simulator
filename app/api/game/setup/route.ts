...
export async function POST(request: Request) {
  try {
    ...
    if ((session.state as { setupComplete?: boolean }).setupComplete) return NextResponse.json({ error: "This business is already open. Start a new business to change the setup." }, { status: 409 });
