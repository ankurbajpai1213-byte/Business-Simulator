  ...
  try {
    ...
    for (let i = 0; i < span; i += 1) {
      ...
      if (!lastDayOfSpan) {
        ...
        if (nextState.inventory < 14) stop = { day: nextState.day, reason: "stockout", message: "Supplies are nearly gone. Another busy day and you will be turning people away while the rent still has to be paid." };
        else if (nextState.profit < 0 && nextState.cash < Math.abs(nextState.profit) * 5) stop = { day: nextState.day, reason: "cash-critical", message: "Cash is running dangerously low. At this rate the business has only a few days left." };
    ...
    nextState.pendingDelayedEffects = [...(nextState.pendingDelayedEffects ?? []), ...newEffects];
    const milestoneUpdate = shouldShowMilestone(nextState.day, nextState.cumulativeProfit, nextState.milestones);
    nextState.milestones = [...new Set([...nextState.milestones, ...milestoneUpdate.timeMilestones, ...(milestoneUpdate.profitMilestone ? [milestoneUpdate.profitMilestone] : [])])];
    const event = nextState.currentEvent ?? generateEvent(nextState);
    nextState.currentEvent = event;
    const status = nextState.cash <= 0 ? "bankrupt" : nextState.day > RUN_LENGTH_DAYS && nextState.cumulativeProfit > 0 ? "won" : nextState.day > RUN_LENGTH_DAYS ? "completed" : "active";
    ...
    const { error: eventError } = await supabase.from("game_events").insert({
      ...
      payload: { decision: primary, decisions: requested, eventOption: resolvedEventOption, resolvedEventTitle, resolvedEventId, eventId: event?.id ?? null, revenue: nextState.revenue, profit: nextState.profit, customers: nextState.customers, totalCustomers: nextState.totalCustomers, cashBefore: turnCashBefore, cash: nextState.cash, turnSpend, reputation: nextState.reputation, staff: nextState.staff, quality: nextState.quality, inventory: nextState.inventory, priceIndex: nextState.priceIndex, cumulativeRevenue: nextState.cumulativeRevenue, cumulativeProfit: nextState.cumulativeProfit, milestones: nextState.milestones, delayedEffectsQueued: newEffects, delayedEffectsApplied: dueEffects, dayMessage: nextState.lastDayMessage, spanDays: span, spanReport: report, status },
    });
