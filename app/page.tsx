...
export default function Home() {
  ...
  const stockDays = state.customers > 0 ? Math.max(0, state.inventory / Math.max(1, state.customers / 9)).toFixed(1) : "—";
  const stockTone = state.inventory < 20 ? "negative" : state.inventory > 82 ? "" : "positive";
  ...
  return <main className="shell"><div className="container">
    ...
    <section className="grid metrics"><Metric label="Cash" value={formatINR(state.cash)} /><Metric label="Revenue" value={formatINR(state.revenue)} /><Metric label="Profit" value={formatINR(state.profit)} good={state.profit >= 0} /><Metric label="Customers" value={state.customers.toLocaleString("en-IN")} /><Metric label="Reputation" value={`${Math.round(state.reputation)}/100`} /><Metric label="Stock" value={`${Math.round(state.inventory)}/100`} good={stockTone === "positive" ? true : stockTone === "negative" ? false : undefined} /></section>
    ...
    <div className="grid lower-grid"><section className="panel"><div className="eyebrow">Your business</div><h2>Where things stand</h2><div className="market"><Row label="Service capacity" value={`${state.serviceCapacity}/day`} /><Row label="Staff" value={`${state.staff}/100`} /><Row label="Quality" value={`${state.quality}/100`} /><Row label="Stock level" value={`${Math.round(state.inventory)}/100`} /><Row label="Stock cover" value={`${stockDays} days`} /><Row label="Today's wastage" value={formatINR(state.wastageToday)} /><Row label="Monthly rent" value={formatINR(location?.rentMonthly ?? 0)} /></div></section><section className="panel menu-card"><div className="eyebrow">Your menu</div><h2>What you sell</h2><div className="chip-list">{state.menu.map(id => <span key={id}>{MENU_ITEMS.find(x => x.id === id)?.name}</span>)}</div></section></div>
  </div>
