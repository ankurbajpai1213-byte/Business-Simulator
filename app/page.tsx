...
export default function Home() {
  ...
  const load = async () => {
    try {
      const p = await fetch("/api/player");
  ...
  const startPlayer = async () => {
    ...
    try {
      const r = await fetch("/api/player", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name }) });
      const d = await r.json();
  ...
  const finishDay = async () => {
    ...
    try {
      ...
      const next = d.state as GameState;
      setState(next); setSelected(null); setEventOption(null);
  ...
  const newBusiness = async () => {
    ...
    try {
      ...
      const d = await r.json();
      if (!r.ok) throw new Error(d.error || "Unable to start a new business.");
      setState(d.state as GameState); setSelected(null); setEventOption(null); setCelebration(null);
  ...
  return <main className="shell"><div className="container">
    <header className="topbar">
      <div><div className="brand">Business Simulator</div><div className="subtle">Hi {player.display_name} · {location?.name} · {format?.name}</div></div>
      <div className="top-actions"><button className="secondary compact" onClick={() => setHistoryOpen(true)}>Past days</button><button className="secondary compact" onClick={() => setMilestonesOpen(true)}>🏆 {state.milestones.length}</button><button className="secondary compact" onClick={newBusiness} disabled={busy}>New business</button></div>
    </header>
  ...
  {feedbackOpen && !feedbackSent && <FeedbackModal onSubmit={submitFeedback} onSkip={() => submitFeedback({ skipped: true })} />}
  </main>;
}

function NameScreen({ name, setName, onStart, busy, error }: { name: string; setName: (v: string) => void; onStart: () => void; busy: boolean; error: string }) {
  return <main className="shell"><div className="container loading"><div className="brand">Business Simulator</div><div className="hero"><div className="day-pill">WELCOME</div><h1>What should we call you?</h1><p>You’re about to build and run your own business.</p><input className="name-input" value={name} maxLength={60} onChange={e => setName(e.target.value)} onKeyDown={e => e.key === "Enter" && onStart()} placeholder="Your name" autoFocus /><button className="primary" onClick={onStart} disabled={busy || !name.trim()}>{busy ? "Getting ready…" : "Let’s go →"}</button>{error && <div className="notice">{error}</div>}</div></div></main>;
}
...
function SetupScreen({ onComplete }: { onComplete: (state: GameState) => void }) {
  const [capital, setCapital] = useState(1000000); const [location, setLocation] = useState<Location>("high-footfall"); const [format, setFormat] = useState<BusinessFormat>("small-cafe");
  const [menu, setMenu] = useState<MenuItemId[]>(["filter-coffee", "instant-coffee", "masala-chai", "bun-maska", "vada-pav", "veg-sandwich", "espresso"]); const [busy, setBusy] = useState(false); const [error, setError] = useState("");
  ...
  const setupCost = selectedFormat.cost + menuCost + 50000 + Math.max(30000, menu.length * 15000); const reserve = capital - setupCost;
  ...
  return <main className="shell"><div className="container setup-container"><section className="hero setup-hero"><div className="day-pill">DAY 0</div><h1>Build your business before you open it.</h1><p>Choose your capital, location, format and menu. What survives setup becomes your working cash.</p></section>
    <SetupSection number="1" title="Starting capital" help="How much are you willing to put into this business?"><div className="actions capital-grid">{CAPITAL_OPTIONS.map(v => <button key={v} className={`action capital-option ${capital === v ? "selected" : ""}`} onClick={() => setCapital(v)}><strong>{capital === v ? "✓ " : ""}{formatINR(v)}</strong><small>{CAPITAL_CAPTIONS[v]}</small></button>)}</div></SetupSection>
    ...
    <SetupSection number="3" title="Choose your business format" help="Your format sets capacity and the infrastructure you can afford."><div className="actions option-grid">{FORMAT_OPTIONS.map(o => <button key={o.id} className={`action ${format === o.id ? "selected" : ""}`} onClick={() => setFormat(o.id)}><strong>{format === o.id ? "✓ " : ""}{o.name}</strong><small>{o.description}</small><em>Setup {formatINR(o.cost)} · {o.capacity}/day</em></button>)}</div></SetupSection>
    <SetupSection number="4" title="Build your menu" help="Pick the products you want to sell. A larger menu costs more to establish and operate."><div className="actions menu-grid">{MENU_ITEMS.map(item => { const available = supports(item.id); const selected = menu.includes(item.id); return <button key={item.id} className={`action ${selected ? "selected" : ""} ${!available ? "locked" : ""}`} onClick={() => toggle(item.id)} disabled={!available}><strong>{selected ? "✓ " : ""}{item.name}</strong><small>{available ? `Setup ${formatINR(item.setupCost)} · Weekly ${formatINR(item.weeklyCost)}` : item.infrastructure === "kitchen" ? "Needs a full kitchen" : "Needs beverage equipment"}</small></button>; })}</div></SetupSection>
    <section className="panel review-card"><div className="eyebrow">Your opening plan</div><h2>{formatINR(capital)} starting capital</h2><div className="market"><Row label="Setup investment" value={formatINR(setupCost)} /><Row label="Working cash" value={formatINR(reserve)} /><Row label="Monthly rent" value={formatINR(LOCATION_OPTIONS.find(x => x.id === location)?.rentMonthly ?? 0)} /><Row label="Menu items" value={`${menu.length}`} /></div>{error && <div className="notice">{error}</div>}<button className="primary" onClick={launch} disabled={busy || menu.length < 1 || reserve <= 0}>{busy ? "Setting up…" : "Open my business →"}</button></section>
  </div></main>;
}

function SetupSection({ number, title, help, children }: { number: string; title: string; help: string; children: React.ReactNode }) { return <section className="panel setup-section"><div className="section-head"><div><div className="eyebrow">Step {number}</div><h2>{title}</h2></div></div><p className="section-help">{help}</p>{children}</section>; }
...
function Modal({ children, onClose }: { children: React.ReactNode; onClose: () => void }) { return <div className="modal-backdrop" onMouseDown={e => e.target === e.currentTarget && onClose()}><div className="modal"><button className="modal-close" onClick={onClose}>×</button>{children}</div></div>; }

function HistoryModal({ state, onClose }: { state: GameState; onClose: () => void }) {
  const items = [...state.dayHistory].reverse();
  return <Modal onClose={onClose}><div className="feedback-modal"><div className="eyebrow">Your journey</div><h2>Past days</h2><p>See what you decided and what happened next.</p><div className="history-list">{items.length ? items.map(item => <div className="history-item" key={`${item.day}-${item.decision}`}><strong>Day {item.day} · {decisionName(item.decision)}</strong><span>{formatINR(item.profit)} · {item.customers} customers · {formatINR(item.cashAfter)} cash</span><small>Reputation {Math.round(item.reputation)}/100 · Price index {item.priceIndex}</small></div>) : <p>No completed days yet.</p>}</div></div></Modal>;
}

function MilestoneModal({ state, onClose }: { state: GameState; onClose: () => void }) {
  const all = [
    ["open-business", "🏪 Open for Business", "Start your first business."], ["first-sale", "🧾 First Sale", "Make your first revenue."], ["first-customer", "👤 First Customer", "Serve your first customer."], ["100-customers", "👥 100 Customers", "Serve 100 customers."], ["500-customers", "👥 500 Customers", "Serve 500 customers."], ["1000-customers", "👥 1,000 Customers", "Serve 1,000 customers."], ["revenue-1l", "💰 ₹1L Revenue", "Reach ₹1 lakh cumulative revenue."], ["revenue-5l", "💰 ₹5L Revenue", "Reach ₹5 lakh cumulative revenue."], ["revenue-10l", "💰 ₹10L Revenue", "Reach ₹10 lakh cumulative revenue."], ["first-profit", "📈 First Profit", "Finish a profitable day."], ["profit-streak-3", "🔥 Momentum", "Three profitable days in a row."], ["profit-streak-5", "🔥 Hot Streak", "Five profitable days in a row."], ["crisis-survived", "🧯 Crisis Survived", "Handle a business event and stay afloat."], ["bounce-back", "🔄 Bounce Back", "Recover from a loss-making day."], ["reputation-60", "⭐ Getting Noticed", "Reach 60 reputation."], ["reputation-80", "⭐ Local Favourite", "Reach 80 reputation."], ["day-5", "📅 Five Days", "Survive the first five days."], ["day-10", "📅 Ten Days", "Survive ten days."], ["day-30", "🏆 Built to Last", "Survive thirty days."] as const
  ];
  return <Modal onClose={onClose}><div className="feedback-modal"><div className="eyebrow">Your progress</div><h2>Milestones</h2><p>{state.milestones.length} achieved so far.</p><div className="history-list">{all.map(([id, title, desc]) => <div className={`history-item ${state.milestones.includes(id) ? "selected" : ""}`} key={id}><strong>{state.milestones.includes(id) ? "✓ " : "🔒 "}{title}</strong><span>{desc}</span></div>)}</div></div></Modal>;
}

function FeedbackModal({ onSubmit, onSkip }: { onSubmit: (a: { ease: string; gameplay: string; realism: string; decisions: string; continuePlaying: string; comment?: string }) => void; onSkip: () => void }) {
  const [ease, setEase] = useState(""); const [gameplay, setGameplay] = useState(""); const [realism, setRealism] = useState(""); const [decisions, setDecisions] = useState(""); const [continuePlaying, setContinuePlaying] = useState(""); const [comment, setComment] = useState("");
  const q = (label: string, value: string, set: (v: string) => void, options: string[]) => <div className="feedback-question"><strong>{label}</strong><div className="choice-row">{options.map(o => <button key={o} className={`choice ${value === o ? "selected" : ""}`} onClick={() => set(o)}>{o}</button>)}</div></div>;
