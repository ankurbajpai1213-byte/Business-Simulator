...
function deriveStage(state: GameState | null) {
  ...
  const manager = Boolean(state.manager);
  ...
  if (manager || milestones.has("manager-hired")) return 2;
  if ((state.day ?? 1) >= 30 && reputation >= 60 && profit > 0) return 1;
  return 0;
}

export default function ProgressionDock() {
  ...
  const progress = useMemo(() => {
    ...
    return Math.min(95, Math.max(8, Math.round(((day % 30) / 30) * 70 + (state?.reputation ?? 50) * 0.2)));
  }, [current, state]);
  ...
  async function sendFeedback() {
    ...
    try {
      const r = await fetch("/api/feedback", {
        ...
        body: JSON.stringify({ rating: rating || null, message: message.trim(), context: { stage: STAGES[current].label, day: state?.day ?? null } }),
      });
  ...
  return (
    <>
      <div style={{ position: "fixed", right: 16, bottom: 16, zIndex: 80, fontFamily: "inherit" }}>
        ...
        {open && (
          <div style={panelStyle}>
            ...
            <div style={{ height: 6, borderRadius: 999, background: "rgba(0,0,0,.10)", margin: "14px 0" }}>
              <div style={{ width: `${progress}%`, height: "100%", borderRadius: 999, background: "currentColor", transition: "width .3s" }} />
            </div>
            {STAGES.map((stage, i) => (
              <div key={stage.id} style={{ display: "flex", gap: 10, alignItems: "flex-start", padding: "7px 0", opacity: i <= current ? 1 : .45 }}>
                <span style={{ width: 20, height: 20, borderRadius: 999, display: "grid", placeItems: "center", border: "1px solid currentColor", fontSize: 11 }}>{i < current ? "✓" : i === current ? "•" : i + 1}</span>
                <div><strong>{stage.label}</strong><div style={{ fontSize: 11, opacity: .7 }}>{stage.description}</div></div>
              </div>
            ))}
      ...
      {feedback && (
        <div onClick={() => setFeedback(false)} style={backdropStyle}>
          <div onClick={e => e.stopPropagation()} style={{ ...panelStyle, width: "min(460px, calc(100vw - 32px))" }}>
            ...
            <div style={{ display: "flex", gap: 6, margin: "14px 0" }}>
              {[1,2,3,4,5].map(n => <button key={n} onClick={() => setRating(n)} style={{ ...buttonStyle, opacity: rating >= n ? 1 : .45 }}>{n}★</button>)}
            </div>
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 12 }}>
              <button onClick={() => setFeedback(false)} style={buttonStyle}>Close</button>
              <button onClick={sendFeedback} disabled={!message.trim() || sending} style={{ ...buttonStyle, fontWeight: 800 }}>{sending ? "Sending…" : "Send feedback"}</button>
            </div>
          </div>
        </div>
      )}
...
const buttonStyle: React.CSSProperties = { border: "1px solid rgba(0,0,0,.14)", background: "rgba(255,255,255,.92)", borderRadius: 999, padding: "8px 12px", cursor: "pointer", boxShadow: "0 4px 14px rgba(0,0,0,.08)" };
const panelStyle: React.CSSProperties = { background: "rgba(255,255,255,.97)", color: "#171717", border: "1px solid rgba(0,0,0,.12)", borderRadius: 18, padding: 18, boxShadow: "0 18px 50px rgba(0,0,0,.18)", width: 340, maxWidth: "calc(100vw - 32px)" };
const backdropStyle: React.CSSProperties = { position: "fixed", inset: 0, zIndex: 100, display: "grid", placeItems: "center", background: "rgba(0,0,0,.42)", padding: 16 };
