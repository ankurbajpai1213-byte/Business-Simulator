...
function eligible(ctx: ScenarioContext, id: GameEventId) {
  if (ctx.recentEventIds.slice(-EVENT_COOLDOWN).includes(id)) return false;
...
function weight(ctx: ScenarioContext, id: GameEventId) {
  ...
  if (id === "competitor-promotion" && ctx.state.priceIndex > 115) w += 1.5;
  if (id === "supplier-increase" && ctx.state.supplierCostMultiplier <= 1.02) w += 1;
  if (id === "bulk-order" && ctx.state.serviceCapacity >= 200) w += 1.5;
  if (id === "local-event" && ctx.state.marketing >= 45) w += 1;
  if (id === "rain") w = 0.35;
  return w;
}
