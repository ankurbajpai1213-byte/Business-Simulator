...
export function createConfiguredState(input: { capital: number; location: Location; format: BusinessFormat; menu: MenuItemId[] }): GameState {
  ...
  if (!CAPITAL_OPTIONS.includes(input.capital)) throw new Error("Invalid capital."); if (result.reserve <= 0) throw new Error("Startup configuration exceeds available capital."); if (input.menu.length < 1) throw new Error("Choose at least one menu item.");
