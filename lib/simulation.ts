...
export function calculateSetup(capital: number, location: Location, format: BusinessFormat, menu: MenuItemId[]) {
  ...
  if (selected.some(item => item.infrastructure === "beverage") && format === "takeaway") throw new Error("A takeaway kiosk needs a beverage upgrade before serving espresso-based drinks.");
  if (selected.some(item => item.infrastructure === "kitchen") && format !== "full-cafe") throw new Error("Kitchen items require a full-service café format.");
...
export function createConfiguredState(input: { capital: number; location: Location; format: BusinessFormat; menu: MenuItemId[] }): GameState {
  ...
  return { ...INITIAL_STATE, setupComplete: true, capital: input.capital, cash: result.reserve, location: input.location, format: input.format, menu: input.menu, setupCost: result.setupCost, staff: result.staff, serviceCapacity: result.serviceCapacity, inventory: 100 };
}
function menuStats(menu: MenuItemId[]) { return menu.reduce((acc, id) => { const item = MENU_ITEMS.find(x => x.id === id)!; return { ticket: acc.ticket + item.ticketImpact, demand: acc.demand + item.demandImpact, weeklyCost: acc.weeklyCost + item.weeklyCost }; }, { ticket: 0, demand: 0, weeklyCost: 0 }); }
export function applyDecision(state: GameState, decision: Decision): GameState { const next = { ...state }; switch (decision) { case "raise-price": next.priceIndex = Math.min(140, next.priceIndex + 8); break; case "marketing": if (next.cash >= 10000) { next.cash -= 10000; next.marketing = Math.min(100, next.marketing + 12); } break; case "hire": if (next.cash >= 18000) { next.cash -= 18000; next.staff = Math.min(100, next.staff + 12); next.serviceCapacity += 15; } break; case "quality": if (next.cash >= 12000) { next.cash -= 12000; next.quality = Math.min(100, next.quality + 8); } break; case "inventory": if (next.cash >= 8000) { next.cash -= 8000; next.inventory = Math.min(100, next.inventory + 20); } break; } return next; }
export function advanceDay(state: GameState): GameState { const location = LOCATION_OPTIONS.find(x => x.id === state.location)!; const menu = menuStats(state.menu); const demandFactor = location.demand * (state.reputation / 100) * (state.quality / 100) * (state.staff / 100) * (1 + state.marketing / 500) * (100 / state.priceIndex) * (1 + menu.demand / 100); const customers = Math.max(0, Math.min(state.serviceCapacity, Math.round(180 * demandFactor * (0.94 + Math.random() * 0.12)))); const averageTicket = (300 + menu.ticket) * (state.priceIndex / 100); const revenue = Math.round(customers * averageTicket); const rent = location.rent; const payroll = 19000 + Math.round(state.staff * 95); const inventoryCost = Math.round(revenue * 0.30); const marketingSpend = Math.round(state.marketing * 90); const weeklyMenuCost = Math.round(menu.weeklyCost / 7); const servicePenalty = state.customers > state.serviceCapacity * 0.9 ? 2500 : 0; const operatingCost = rent + payroll + inventoryCost + marketingSpend + weeklyMenuCost + servicePenalty; const profit = revenue - operatingCost; return { ...state, day: state.day + 1, cash: Math.round(state.cash + profit), revenue, profit, customers, reputation: Math.max(0, Math.min(100, state.reputation + (profit > 0 ? 1 : -1))), inventory: Math.max(0, state.inventory - Math.round(customers / 8)), marketing: Math.max(25, state.marketing - 2) }; }
export function generateEvent(state: GameState): GameEvent | null { if (state.day < 4 || state.currentEvent) return null; if (state.day % 7 === 0 && state.reputation < 75) return { id: "bad-review", title: "Bad review", severity: 2, narrative: "A local food page has posted: ‘Great coffee, terrible waiting time.’ Your response could affect reputation and future demand.", options: [{ id: "improve-service", title: "Improve service", description: "Invest in capacity. Expensive now, but addresses the underlying problem.", cost: 40000 }, { id: "compensate", title: "Compensate affected customer", description: "Contain the damage cheaply, but the service problem remains.", cost: 15000 }, { id: "ignore-review", title: "Ignore it", description: "No immediate cost, but repeated complaints may damage reputation.", cost: 0 }] }; if (state.day % 5 === 0) return { id: "supplier-increase", title: "Supplier price increase", severity: 1, narrative: "Your primary supplier has announced an 18% increase on key ingredients.", options: [{ id: "accept-supplier", title: "Accept the increase", description: "No disruption, but margins will fall.", cost: 0 }, { id: "switch-supplier", title: "Switch supplier", description: "Pay setup costs now to protect margins later.", cost: 12000 }] }; return null; }
export function applyEvent(state: GameState, optionId: string): GameState { if (!state.currentEvent) return state; const next = { ...state, currentEvent: null, eventHistory: [...state.eventHistory, `${state.day}:${state.currentEvent.id}:${optionId}`] }; switch (state.currentEvent.id) { case "bad-review": if (optionId === "improve-service" && next.cash >= 40000) { next.cash -= 40000; next.serviceCapacity += 25; next.staff = Math.min(100, next.staff + 5); next.reputation = Math.min(100, next.reputation + 2); } else if (optionId === "compensate" && next.cash >= 15000) { next.cash -= 15000; next.reputation = Math.min(100, next.reputation + 1); } else if (optionId === "ignore-review") next.reputation = Math.max(0, next.reputation - 4); break; case "supplier-increase": if (optionId === "switch-supplier" && next.cash >= 12000) next.cash -= 12000; else if (optionId === "accept-supplier") next.quality = Math.max(0, next.quality - 3); break; } return next; }
